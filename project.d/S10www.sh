...
case "$COMMAND" in 
    ...
    external) 
        # settings.php
	if [ -e $WWW_DIR/$PROJECT/sites/default/default.settings.php ] ; then
	    DEFAULT="$WWW_DIR/$PROJECT/sites/default/default.settings.php"
	elif [ -e $WWW_DIR/$PROJECT/sites/default/settings.php ] ; then
	    DEFAULT="$WWW_DIR/$PROJECT/sites/default/settings.php"
	else
	    DEFAULT="$WWW_DIR/$TEMPLATE/sites/default/settings.php"
	fi
	...
	mkdir -p $WWW_DIR/$PROJECT/sites/$PROJECT.$DOMAIN
	sed -i "s#\$db_url = 'mysql://username:password@localhost/databasename'#\$db_url = 'mysqli://$DEFAULT_DB_UN:$DEFAULT_DB_PW@localhost/$PROJECT'#" $DEFAULT > $WWW_DIR/$PROJECT/sites/$PROJECT.$DOMAIN/settings.php 
	...
	if [ -d $WWW_DIR/$PROJECT/sites/all/files ] ; then
        ...
        # htaccess
	copy_if_missing $WWW_DIR/$PROJECT/.htaccess $HOME/${PROJECT}-${COMMAND}/$PROJECT/.htaccess
	;;
